...
export default {
  ...
  created() {
    ...
    let _orderNo = getUrlAttribute(window.location.href, 'orderno') || '' //订单编号
    ...
    } else if (_target == 'orderdetail') {
      //订单详情
      this.$router.replace(`/orderdetail/${_orderNo}`)
    } else {
      // console.log(window.location.href + '\n')
      let _split = window.location.href.split('#/')
      // console.log('\n' + _split)
      if (_split[1].length != 0) {
        console.log('有：' + _split)
  ...
  // destroyed() {
  //   localStorage.removeItem('JINGWEBUSERTOKEN')
  // }
}
</script>
